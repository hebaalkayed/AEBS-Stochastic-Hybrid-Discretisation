...
\begin{document}
	...
	//HERE
	Each enumerated target is bracketed independently. The source cell
	fixes one image interval, and this interval is the same whichever
	target is considered; no density ever moves toward a target. The
	target fixes the window and its middle. The bracket for the
	transition to that target is the smallest and largest landing chance
	over the shared image interval. Repeating this per candidate target
	gives one bracket per transition. The brackets are bounds, not shares
	of a fixed total: a target whose window lies away from the image
	interval receives a bracket small at both ends, and the mass appears
	instead in the transitions to the cells the image covers. The
	candidate targets are the cells beneath the image interval widened by
	$6\sigma_d$ on each side; six is a chosen cutoff, placed so that the
	Gaussian mass beyond the window, of order $10^{-9}$, lies below the
	numerical tolerance of the containment certificate
	(Section~\ref{sec:cs:exp}).
	...
	stochastic dimension, its \emph{grid band}, while the Gaussian places
	some mass outside every bounded interval. A small fraction of each
	step's probability therefore falls outside the grid
	(Figure~\ref{fig:truncation}). The construction truncates: the
	dimension is taken to remain inside the band, and every stochastic
	factor is divided by the in-band mass, so the in-band probabilities
	again total one.
	...
	\begin{figure}[h]
		...
		\begin{tikzpicture}
			...
			{out-of-band tail:\\renormalised away};
		\end{tikzpicture}
		\caption{Truncation of the stochastic tail. The grid band $[b_{\min}, b_{\max}]$
			...
			$\Pi(\mu)$, depends on the prediction $\mu$; the out-of-band tail is
			renormalised away, conditioning the kernel on the band.}
		\label{fig:truncation}
	\end{figure}
	
	Truncating means conditioning: the stored probability of a target
	becomes the probability of reaching it \emph{given} that the landing
	stayed inside the band. A conditional probability is a division, the
	target mass divided by the in-band mass, so the treatment needs a
	divisor. The divisor is itself a landing chance, the same
	formula~\eqref{eq:g} with the whole band as the window,
	\begin{equation}
		\Pi(\mu) = \Phi\!\Bigl(\tfrac{b_{\max}-\mu}{\sigma_d}\Bigr)
		- \Phi\!\Bigl(\tfrac{b_{\min}-\mu}{\sigma_d}\Bigr),
		\label{eq:inband}
	\end{equation}
	and therefore behaves exactly like $g$: it depends on the prediction,
	and it is largest when the prediction sits at the band's midpoint,
	falling on either side. The prediction varies over the image
	interval, so the divisor varies with it, and a single divisor per
	cell would be wrong for most states in the cell. The divisor is
	therefore bracketed the same way as $g$: its smallest value over the
	image interval, $\Pi_{\text{lo}}$, and its largest,
	$\Pi_{\text{hi}}$, found at the interval ends or the band midpoint.
	The division must not shrink the stored bracket. Dividing by a larger
	number gives a smaller result, so the lower end is divided by the
	largest divisor and the upper end by the smallest:
	\begin{equation}
		\bigl[\,m/\Pi_{\text{hi}},\; M/\Pi_{\text{lo}}\,\bigr].
		\label{eq:trunc}
	\end{equation}
	Every continuous state of the cell then has its true conditional
	probability between the stored ends: its numerator lies between $m$
	and $M$, its divisor between $\Pi_{\text{lo}}$ and $\Pi_{\text{hi}}$,
	hence
	\begin{equation}
	...
	Every number in the generated model now has a source. A deterministic
	dimension has no randomness: its whole outcome sits at one point, so
	its factor ends are $0$ or $1$, and when that point falls on a shared
	cell boundary, ownership of the boundary decides the value; hence the
	strict test in~\eqref{eq:flo}. The stochastic dimension spreads its
	outcome as a density, and a single point carries none of a density's
	mass, so boundary ownership changes nothing and the factor is
	fractional. A transition's interval is the product~\eqref{eq:product}
	of one factor per dimension, and every fractional entry in the model
	is the stochastic factor showing through that product
