...
\begin{document}
	...
	//here
	Algorithm~\ref{alg:imagebox} computes the image box and performs the
	noise check of \hyperref[ass:sysclass]{Assumption~(i)}. It evaluates
	the dynamics at every corner of the source cell and records, per
	dimension, the smallest and largest next value and the smallest and
	largest noise across the corners. The corner coordinates are the
	stored edge values of~\eqref{eq:bins} themselves, not values
	reconstructed from a cell centre and a half-width. Reconstruction can
	differ from the stored edges in the last bits of floating-point
	arithmetic, and the boundary reasoning of~\eqref{eq:flo} is exact
	only when the corners are evaluated at the delimiting values. The
	upper edges are among the corner coordinates, so the corner set spans
	the closure of the half-open cell, and the returned image interval
	bounds the image of every state of the cell, boundary included.
	...
	\begin{algorithm}[H]
		...
		\begin{algorithmic}[1]
			...
			\EndFor
			\For{each dimension $d$}
			\If{$\sigma^{\text{hi}}_d - \sigma^{\text{lo}}_d > 10^{-12}$} \textbf{raise} (noise not constant across the cell) \EndIf
			\EndFor
			\State \Return $[\mu^{\text{lo}}, \mu^{\text{hi}}]$ and the common noise $\sigma = \sigma^{\text{lo}}$
		\end{algorithmic}
	\end{algorithm}
	
	Algorithm~\ref{alg:boxprob} returns the
	extrema~\eqref{eq:mlo}--\eqref{eq:Mhi} of the landing chance over the
	image interval.
	
	\begin{algorithm}[H]
		\caption{$\textsc{BoxProbRange}$: range of the landing chance over the image interval (adapting~\cite[Eq.~3.11]{Soudjani2013}).}
		\label{alg:boxprob}
		\begin{algorithmic}[1]
			\Require image interval $[\mu^{\text{lo}}, \mu^{\text{hi}}]$, target window $\ell^j, h^j$, noise $\sigma$
			\Ensure $(m, M)$, the smallest and largest value of $g$ over the image interval
	...
	Algorithm~\ref{alg:intervals} assembles one row of the IMDP. It
	builds the image box, applies the terminal routing and the escape
	routing, and computes the factor bracket of every dimension.
	Stochastic factors come from Algorithm~\ref{alg:boxprob} with the
	truncation division~\eqref{eq:trunc}; deterministic factors
	from~\eqref{eq:flo}. The brackets multiply into the transition
	intervals~\eqref{eq:product}.
	...
	\begin{algorithm}[H]
		...
		\begin{algorithmic}[1]
			...
			\State $\textit{terminalPossible} \gets$ image partly inside the terminal region
			\If{the image lies wholly outside the grid in a deterministic dimension} \Return $\{\text{sink} : (1, 1)\}$ \EndIf
			\State $\textit{escapePossible} \gets$ image reaches the grid's top edge, or crosses below its bottom edge, in a deterministic dimension (terminal edge excepted)
			\State $\Pi_{\text{lo}}, \Pi_{\text{hi}} \gets$ extrema of~\eqref{eq:inband} over the image interval (interval ends; band midpoint for $\Pi_{\text{hi}}$ when the interval contains it)
			\For{each dimension $d$}
			\If{$\sigma_d > 0$} \Comment{stochastic dimension}
			\State enumerate cells $k$ within $[\mu^{\text{lo}}_d, \mu^{\text{hi}}_d]$ extended by $n_{\mathrm{cut}}\sigma_d$
			\State per $k$: $(m, M) \gets \textsc{BoxProbRange}(\mu^{\text{lo}}_d, \mu^{\text{hi}}_d, \text{bin}^k_{\text{lo}}, \text{bin}^k_{\text{hi}}, \sigma_d)$; divide as in~\eqref{eq:trunc}; discard if $M$ below tolerance
			\Else \Comment{deterministic dimension; enumeration ends at the terminal boundary}
			\State enumerate cells $k$ the image touches; per $k$: $f_{\text{lo}} \gets 1$ if the image lies wholly inside the half-open cell $k$, else $0$; factor $(k, f_{\text{lo}}, 1)$
			\EndIf
			\EndFor
			\State $\mathcal{T} \gets \{\,\text{flat}(k_1, \dots, k_n) : (\textstyle\prod_d f^d_{\text{lo}},\ \prod_d f^d_{\text{hi}})\,\}$ over all factor combinations, products below tolerance dropped
