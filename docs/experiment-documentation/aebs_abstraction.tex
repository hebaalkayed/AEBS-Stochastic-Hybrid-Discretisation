...
\begin{document}
	...
	\end{itemize}
	The stored probability of a target cell is then the conditional
	probability of reaching it given that the landing lies inside the
	band, the quotient $g(\mu)/\Pi(\mu)$. The quotient must remain valid
	for every continuous state of the source cell, and the predictions of
	those states span the image interval, so the divisor is bracketed
	over the image interval. The function $\Pi$ is largest when the
	prediction is at the midpoint of the band and decreases on either
	side. Its smallest value over the image interval, $\Pi_{\text{lo}}$,
	is therefore attained at an end of the interval; its largest,
	$\Pi_{\text{hi}}$, at the midpoint of the band if the interval
	contains it, and at the nearer end otherwise. The stored factor
	divides the lower end by the largest divisor and the upper end by the
	smallest,
	\begin{equation}
		\bigl[\,m/\Pi_{\text{hi}},\; M/\Pi_{\text{lo}}\,\bigr].
		\label{eq:trunc}
	\end{equation}
	For every prediction $\mu$ in the image interval,
	$m \le g(\mu) \le M$ and
	$\Pi_{\text{lo}} \le \Pi(\mu) \le \Pi_{\text{hi}}$, hence
	\begin{equation}
