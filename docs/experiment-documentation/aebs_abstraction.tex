...
\begin{document}
	...
	This work is accordingly positioned among the verification frameworks for learning-enabled autonomous systems, surveyed under the Verified AI agenda of~\cite{SSS20}. Compositional verification derives system-level guarantees from component-level guarantees and assumptions between the components~\cite{pasareanu2023assumptiongenerationverificationlearningenabled}. Falsification searches the input space of a closed-loop system for executions that violate a specification, and has been applied to an automatic emergency braking system with DNN perception~\cite{dreossi2018compositionalfalsificationcyberphysicalsystems}. Runtime assurance in the Simplex tradition pairs an untrusted controller with a verified fallback and a monitor that switches between them~\cite{simplex}. These frameworks operate on a system-level model that they take as given. The gap this programme addresses is the automatic construction of that model from the concrete dynamics of the system.
	...
	This section fixes, in the standard terms of the field, the ingredients on which the abstraction pipeline of Section~\ref{sec:abs} and the case study of Section~\ref{sec:cs} rest. These are the class of continuous models we abstract from, the safety question asked of them, the partition approach by which such models are made finite, the interval-valued finite model we abstract to, and the framework by which verification results transfer between the two.
	
	\subsection{Discrete-time stochastic hybrid systems}
	\label{sec:bg:dtshs}
	
	The source models of the abstraction are the stochastic hybrid systems of Section~\ref{sec:intro}, given in discrete time. Following the formalisation of~\cite[Def.~1.2]{LSAZ22}, a \emph{discrete-time stochastic hybrid system} (dt-SHS) is a tuple
	\begin{equation}
	...
	where
	\begin{itemize}
		\item $Q = \{q_1, \ldots, q_p\}$ is a finite set of discrete modes;
		\item $n : Q \to \mathbb{N}_{\ge 1}$ assigns to each mode a continuous-state dimension;
		\item $X \subseteq \bigcup_{q \in Q} \{q\} \times \R^{n(q)}$ is the hybrid state space with Borel $\sigma$-algebra $\mathcal{B}(X)$;
		\item $U \subseteq \R^m$ is a Borel input space;
		\item $T_x : \mathcal{B}(X) \times X \times U \to [0,1]$ is a conditional stochastic kernel;
		\item $Y$ is a Borel output space and $h : X \to Y$ is a measurable output map.
	\end{itemize}
	...
	The safety question asked of such a system is whether it remains within a designated safe region over a finite time horizon. For a dt-SHS $\Sigma$ with state space $X$, the \emph{probabilistic invariance} problem~\cite{APLS08} is to compute, for each initial state $x_0 \in X$, the probability
	\begin{equation}
	...
	where
	\begin{itemize}
		\item $A \subseteq X$ is the Borel set of safe states;
		\item $N$ is the finite horizon, counted in steps;
		\item $x(k)$ is the state at step $k$ under the kernel $T_x$.
	\end{itemize}
	The complementary quantity is the \emph{bounded-reachability} probability
	\begin{equation}
		r_{x_0}(A) \;=\; 1 - p_{x_0}(A) \;=\; \Prob\bigl(\,x(k) \in X \setminus A\ \text{for some } k \in [0, N] \,\big|\, x(0) = x_0\,\bigr),
		\label{eq:reach}
	\end{equation}
	where $X \setminus A$ is the unsafe set. The safety properties verified in Section~\ref{sec:cs} are bounded-reachability probabilities of this form. Both quantities are characterised by a backward recursion whose every step integrates against the kernel~\cite[Thm.~1]{APLS08}; these integrals are generally intractable in closed form, so neither quantity is computable directly on the continuous system.
	
	\subsection{Partition-based abstraction}
	\label{sec:bg:grid}
	
	A finite model of a dt-SHS is obtained by partitioning its continuous state space into finitely many cells and treating each cell as one state. On the finite model the backward recursion of Section~\ref{sec:bg:dtshs} becomes a finite computation. A \emph{partition} of the state space is a finite family of sets satisfying
	\begin{equation}
		X \;=\; \bigcup_{i=1}^{M} A_i, \qquad A_i \cap A_j = \emptyset \ \text{ for } i \neq j,
		\label{eq:partition-bg}
	\end{equation}
	where
	\begin{itemize}
		\item $M$ is the number of cells;
		\item $A_1, \ldots, A_M$ are the \emph{cells}, pairwise disjoint measurable subsets of $X$.
	\end{itemize}
	
	By~\eqref{eq:dtshs-kernel}, one step from a continuous state inside a source cell lands in a given target cell with a probability determined by the kernel. This probability generally varies as the state ranges over the source cell. The classical construction ignores the variation: it fixes one \emph{representative point} per cell and adopts the kernel value at that point as the cell-to-cell transition probability~\cite{LSAZ22},
	\begin{equation}
		P(i, a, j) \;=\; T_x\bigl(A_j \,\big|\, z_i,\; a\bigr),
		\label{eq:reppoint}
	\end{equation}
	where
	\begin{itemize}
		\item $z_i \in A_i$ is the representative point chosen in the source cell $A_i$;
		\item $P(i, a, j)$ is the resulting transition probability from cell $A_i$ to cell $A_j$ under input $a$.
	\end{itemize}
	When the input set is finite, the model so produced is a \emph{Markov decision process} (MDP), a finite model with states, actions, and one exact probability per transition~\cite{KNP25}. Retaining the within-cell variation instead, as a range of admissible values for each transition, leads to the interval-valued target model of Section~\ref{sec:bg:imdp}.
	
	\subsection{Interval Markov decision processes}
	\label{sec:bg:imdp}
	
	The target of the abstraction stores the within-cell variation of Section~\ref{sec:bg:grid} as an interval attached to each transition. Following~\cite{GLD00}, an \emph{Interval Markov Decision Process} (IMDP, also called a \emph{bounded-parameter MDP}) is a tuple
	\begin{equation}
		\mathcal{I} \;=\; (S,\; U,\; \check P,\; \hat P),
		\label{eq:imdp}
	\end{equation}
	where
	\begin{itemize}
		\item $S$ is a finite set of states and $U$ a finite set of actions;
		\item $\check P, \hat P : S \times U \times S \to [0,1]$ are the lower-bound and upper-bound transition functions, with $\check P(s,a,s') \le \hat P(s,a,s')$ for every triple.
	\end{itemize}
	...
	For the abstraction of a dt-SHS, the IMDP takes one state per cell of~\eqref{eq:partition} and its actions from the finite input set. The interval attached to the transition from cell $A_i$ to cell $A_j$ under action $a$ is the range of the cell-to-cell probability over the source cell,
	\begin{equation}
		\check P(i, a, j) \;=\; \min_{s \in A_i} T_x\bigl(A_j \,\big|\, s, a\bigr), \qquad
		\hat P(i, a, j) \;=\; \max_{s \in A_i} T_x\bigl(A_j \,\big|\, s, a\bigr),
		\label{eq:extrema}
	\end{equation}
	where the minimum and the maximum are the smallest and the largest value the cell-to-cell probability takes as the state $s$ ranges over the source cell $A_i$; both are attained under the continuity assumption of Section~\ref{sec:abs}. This is the interval-valued abstraction of~\cite{LAB15}.
	
	\subsection{Robust verification of interval models}
	\label{sec:bg:robust}
	
	Verification of an interval model must account for every exact model that its intervals admit. A \emph{selection} of the IMDP is a choice, for each state and action, of one transition distribution consistent with the intervals. Properties are expressed in probabilistic computation tree logic (PCTL), the specification language of probabilistic model checking, in which both the invariance and the bounded-reachability probabilities of Section~\ref{sec:bg:dtshs} are expressible~\cite{KNP25}. \emph{Robust verification} of a PCTL property over an IMDP computes the smallest and the largest value, written $P_{\min}$ and $P_{\max}$, that the property attains over the represented set. We call the pair $[P_{\min}, P_{\max}]$ the \emph{bracket} of the property. The value of the property on every MDP of the set, in particular on every selection, lies within the bracket.
	
	The transfer we rely on is stated in~\cite{LAB15}. When the one-step transition probabilities of a system lie, at every step, within the intervals of the IMDP, the bracket computed on the IMDP contains the property value of that system. Containment of the concrete transition probabilities within the abstract intervals is therefore the single hypothesis under which a verification result on the IMDP transfers to the system being abstracted. Section~\ref{sec:abs:sound} makes this hypothesis precise and discharges it.
	In practice the bracket is computed by robust value iteration, the discrete counterpart of the backward recursion of Section~\ref{sec:bg:dtshs}, resolved at each step against the extreme distributions admitted by the intervals; the model checker Storm implements it for interval models~\cite{Storm}.
	...
	This section presents the construction. Section~\ref{sec:abs:partition} partitions the continuous state space into the states of the IMDP; Section~\ref{sec:abs:intervals} computes the probability intervals attached to its transitions; Section~\ref{sec:abs:sound} establishes soundness, by the design of the construction and by a per-transition test against the system's own dynamics. The composition of the per-action rows into the model handed to the verifier is described with the case study (Section~\ref{sec:cs:exp}).
	...
	$[\mu_d^{\text{lo}}, \mu_d^{\text{hi}}]$, so a bracket valid for every continuous state of the cell is
	\begin{equation}
	...
	the probability that the landing lies inside the band. Away from the	ends of the band this probability is $1$ to within the mass beyond the cutoff and the factor is unchanged;	near an end it is smaller than $1$, and the division raises the
	...
	the continuous system. The transfer
	of~\cite{LAB15} (Section~\ref{sec:bg:robust}) reduces this to one
	...
	hypothesis from the design of the construction, and validates
	the implementation by a test that recomputes the true
	probabilities from the system itself.
	
	\paragraph{Containment by design.}
	The corner evaluation~\eqref{eq:imagebox} returns the range of the mean map over the closure of the source cell, so the image box encloses
	...
	satisfies~\eqref{eq:cellcontain}. Under this containment the one-step law of the continuous system is, at every step, a selection of the IMDP, and the verified bracket $[P_{\min}, P_{\max}]$ of Section~\ref{sec:bg:robust} covers the property value of the continuous system~\cite{LAB15}. This holds because the targets of a row cover every successor, the grid cells together with the terminal representative and the safe sink, and because the terminal region is a union of cells.
	...
	\begin{notebox}[title=Scope of the soundness claim]
		The claim covers the system as modelled, under five
		conventions fixed before the abstraction begins. Any
		cross-dimension correlation of the raw physics is removed in
		adopting the factorised kernel~\eqref{eq:kernelfactor}. The
		stochastic dimension is conditioned on the grid
		band~\eqref{eq:quotient}. Mass that leaves the grid in a
		deterministic dimension is absorbed in the safe sink, so a
		run that leaves the modelled window is counted as never
		reaching the terminal region thereafter. Labels are read at
		cell centres (Section~\ref{sec:abs:partition}), so the
		predicate verified is the predicate as quantised on the grid,
		and the same holds for a controller composed with the model
		(Section~\ref{sec:cs:exp}). The implementation omits the mass
		beyond the cutoff and below the tolerance of
		Section~\ref{sec:abs:intervals}. Both sides of the test apply
		the first two conventions, so the test relates the IMDP to
	...
	\begin{itemize}
		...
		\item $\alpha = 0.5$ is a static actuator gain, a modelling choice that attenuates the commanded acceleration and stands in for the vehicle's actuator response;
	...
	where
	\begin{itemize}
		\item $r_g$ is the gap resolution of Table~\ref{tab:grids}.
	\end{itemize}
	Its cells are those whose centre satisfies the \texttt{crash} predicate of Table~\ref{tab:labels}. The terminal boundary of Section~\ref{sec:abs:intervals} is $g = 0$. A successor with $g \le 0$ is routed to the absorbing representative, and a successor with $0 < g < r_g/2$ lands in a grid cell that carries the label \texttt{crash}. The region therefore contains the physical crash set $g \le 0$, so a run the model counts as safe has not crashed.
	...
	The containment test of Section~\ref{sec:abs:sound} was run over every cell of both grids under all three actions, sampling $27$ states per cell. A probability was counted as outside its interval only if it fell outside by more than $10^{-7}$. The tolerance covers two effects, floating-point error in the two computations and the mass omitted by the cutoff and the discarded combinations of Section~\ref{sec:abs:intervals}. Neither run found a violation. By the design argument of Section~\ref{sec:abs:sound}, both IMDPs are sound over-approximations of the kernel~\eqref{eq:kernel} within the scope stated there.
	...
	Every scenario has a fractional $P_{\max}$ at some horizon. At any one horizon the five values are ordered by starting gap, largest at the smallest gap, so the certified worst case reproduces the physical ordering. The horizon at which $P_{\max}$ first reaches $1$ is
	\begin{equation}
	...
	\begin{itemize}
		...
		\item $k_1$ is the smallest queried horizon at which it equals $1$.
	\end{itemize}
	Table~\ref{tab:k1} gives $k_1$ per scenario. A straight line fitted to its five rows has a slope of $1.66$ control periods per metre of starting gap; each metre of starting gap defers $k_1$ by $1.66$ control periods, that is, $0.60$\,m of starting gap per control period.
	...
	In plain terms, the case study delivers certified worst-case crash probabilities for an AEBS under perfect perception. On the medium production grid $P_{\max}$ is $0$ up to a horizon set by the start state and $1$ from the next queried horizon. On the extra-fine Following grid the worst case is strictly between $0$ and $1$ at some horizon for every car-following scenario, the values are ordered by starting gap, and the horizon at which $P_{\max}$ first reaches $1$ grows with the starting gap. Every $P_{\min}$ outside the two start states with bracket $[1,1]$ is $0$, so each bracket is an upper bound on the crash probability of the modelled system and not an estimate of it. The simulation records crash fractions inside the brackets.
	
	These results are the perfect-perception baseline of the wider programme (Section~\ref{sec:intro}). The perception layer of Section~\ref{sec:cs:sys} passes the state through unchanged, and every bracket in this section is conditioned on that. The next stage replaces it with measured perception uncertainty from the DNNs, composed with the same certified plant IMDP, and re-runs the same queries; if the composed model admits every behaviour of the perfect-perception model, its curves cannot lie below those of Figure~\ref{fig:followingcurves}, and the baseline makes the difference measurable. The baseline is also the reference for correlating DNN verification results with whole-system safety and for extracting the perception requirements that matter most for undegraded safety. Those requirements serve both as specifications a perception component must meet and as runtime monitors that detect safety degradation in operation. The wider framework composes the perception and plant pieces by assume--guarantee contracts, so the brackets certified here are the plant side of that composition.
	
	\section{Future work}
	\label{sec:cs:future}
	
	This paper delivers the perfect-perception baseline of Track~1 of the programme of Section~\ref{sec:intro}. Figure~\ref{fig:status} restates the programme with the status of each track. The certified artefacts of Section~\ref{sec:cs}, the two certified configurations and the pipeline that produced them, are the inputs the later tracks build on, and the Following brackets of Section~\ref{sec:cs:extrafine} are the quantitative baseline against which the perception-aware model's brackets will be compared.
	...
	\begin{figure}[h]
		...
		\resizebox{0.95\textwidth}{!}{%
			\begin{tikzpicture}[
				...
				\fill[safeGreen!10, rounded corners=6pt] (-1.4, 0.3) rectangle (10.6, 2.5);
				\fill[accentOrange!8, rounded corners=6pt] (-1.4, -3.3) rectangle (10.6, -0.2);
				\fill[imdpBlue!8, rounded corners=6pt] (10.9, -1.4) rectangle (18.2, 1.15);
				
				\node[font=\small\sffamily\bfseries, text=safeGreen!70!black]
				at (4.8, 2.75) {Track 1: Formal Abstraction (this paper)};
				\node[font=\small\sffamily\bfseries, text=accentOrange!80!black]
				at (4.8, -3.55) {Track 2: DNN Uncertainty Quantification (next)};
				\node[font=\small\sffamily\bfseries, text=imdpBlue!80!black]
				at (14.55, 1.4) {Track 3: Composition and Monitor Synthesis};
		...
		\caption{Status of the programme. Tracks~2 and~3 follow the timeline of Figure~\ref{fig:gantt}.}
		\label{fig:status}
	\end{figure}
	
	The coming year proceeds in four steps (Figure~\ref{fig:gantt}).
	\begin{enumerate}
		\item \textbf{Degradation quantification.} A perception component with prescribed error rates is composed with the certified extra-fine configuration, and the Following family is remeasured end-to-end against the baseline. The prescribed rates are state-independent by construction, so this step reuses the certification machinery of Section~\ref{sec:abs:sound} unchanged and exercises the Track~3 interface early.
		\item \textbf{Uncertainty quantification.} The prescribed component is replaced by a trained DNN with a labelled test dataset, either adopted from an established driving-perception dataset or generated in simulation, following the practice of~\cite{DeepDECS24}. Independent verification techniques partition the test dataset by their joint outcome; one confusion matrix per outcome yields the conditional probability of each perceived state given each true state and outcome, the per-input uncertainty tuples of the programme~\cite{DeepDECS24}. We are experimenting with the publicly released DeepDECS implementation, whose augmentation tool composes these probabilities into a discrete-time Markov chain for PRISM; we will adapt it to act on the transition intervals of our Storm-checked IMDP instead, reusing engineering rather than rebuilding it. Measured perception error varies with the state, which breaks the state-independence assumption of Algorithm~\ref{alg:imagebox}; the interval computation and the containment test must therefore be extended to range over state-dependent noise, and this is the one anticipated change to the pipeline. In the composed model the tuples enter the transition intervals alongside the discretisation over-approximation of this paper, so the interval target of Section~\ref{sec:bg:imdp} carries both uncertainty kinds, as it was chosen to do.
		\item \textbf{Monitor synthesis and validation.} Track~3 composes the tuples with the certified IMDP through the perception-aware construction step of Section~\ref{sec:intro} and synthesises runtime monitors from the composed model. At run time the monitor reads the verification signals of the perception component, identifies which quantified uncertainty regime the current input falls in, and applies the safety bound computed offline for that regime; when the applicable bound is unacceptable, the monitor overrides the perception output in favour of a verified fallback. The target result is quantitative recovery: verification of the composed system, using the DNN verification results, should recover a substantial part of the degradation measured in the first step.
		\item \textbf{Monitor extension.} The monitor is extended beyond a single perception channel, with fused sensor inputs as the candidate direction.
	\end{enumerate}
	...
	\begin{figure}[h]
		...
		\resizebox{0.95\textwidth}{!}{%
			\begin{ganttchart}[
				...
				title label font=\small\sffamily\color{black!60},
				bar label font=\small\sffamily,
				bar height=0.55,
				bar top shift=0.225,
				bar/.append style={rounded corners=2pt, draw=none, fill=accentOrange!70},
				vgrid={*1{draw=black!12, line width=0.4pt}},
				x unit=0.85cm,
				y unit title=0.6cm,
				y unit chart=0.7cm
				]{1}{13}
				\gantttitle{2026}{4}\gantttitle{2027}{9} \\
				\gantttitle{S}{1}\gantttitle{O}{1}\gantttitle{N}{1}\gantttitle{D}{1}\gantttitle{J}{1}\gantttitle{F}{1}\gantttitle{M}{1}\gantttitle{A}{1}\gantttitle{M}{1}\gantttitle{J}{1}\gantttitle{J}{1}\gantttitle{A}{1}\gantttitle{S}{1} \\
				\ganttbar{Degradation quantification}{1}{2} \\
				\ganttbar{Uncertainty quantification}{3}{5} \\
				\ganttbar[bar/.append style={rounded corners=2pt, draw=none, fill=imdpBlue!70}]{Monitor synthesis and validation}{6}{10} \\
				\ganttbar[bar/.append style={rounded corners=2pt, draw=none, fill=imdpBlue!70}]{Monitor extension; sensor fusion}{10}{13}
			\end{ganttchart}%
		}
		\caption{Timeline for Tracks~2 and~3.}
		\label{fig:gantt}
	\end{figure}
	
	Within the present track, two items remain. First, the nondeterministic lead: a lead that chooses per step from a set of admissible accelerations~\cite{EuroNCAP} requires a second action axis in the discretiser and the composition, over which the checker would quantify adversarially; it is defined in the implementation but not composed. Second, the width structure of the brackets. The zero lower bounds are exact for width-preserving deterministic dynamics (Section~\ref{sec:cs:exp}), so every measured $P_{\min}$ in this paper is $0$ and the reported brackets are one-sided: informative upper bounds, trivial lower bounds. Actuation noise on the ego dynamics would make the ego dimension stochastic and remove the straddling indicators that force the zeros, at the price of a second stochastic dimension in the kernel.
